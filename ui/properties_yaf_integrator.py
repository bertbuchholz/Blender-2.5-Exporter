...
class YAF_PT_render(RenderButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if scene.intg_light_method == "Direct Lighting":
            ...
            col = layout.column()
            col.prop(scene, "intg_pbgi_samples", text= "Samples")
            col.prop(scene, "intg_pbgi_maxSolidAngle", text= "Solid Angle Factor")
            ...
            col.prop(scene, "intg_pbgi_fb_resolution", text = "FB Resolution")
            col.prop(scene, "intg_pbgi_fb_type", text = "FB Type")

            col.prop(scene, "intg_pbgi_spherical_fct_type", text = "Spherical Function Type")
            if scene.intg_pbgi_spherical_fct_type == 'Cube':
                col.prop(scene, "intg_pbgi_sf_resolution", text = "Cube resolution")

            col.prop(scene, "intg_pbgi_dictionary_type", text = "Dicitionary Type")
            if context.scene.intg_pbgi_dictionary_type != 'No_dict':
                col.prop(scene, "intg_pbgi_dict_num_centers", text = "Dict Centers")
                col.prop(scene, "intg_pbgi_dict_sample_fraction", text = "Dict Sample Fraction")
                col.prop(scene, "intg_pbgi_do_dict_stats", text = "Do Dictionary Stats")

            col.prop(scene, "intg_pbgi_variational", text = "Variational")
            col.prop(scene, "intg_pbgi_enable_conversion", text = "Conversion")
            col.prop(scene, "intg_pbgi_disc_scale_factor", text = "Disc Scaling")
            col.prop(scene, "intg_pbgi_node_splat_t", text = "Node Splat Type")
            col.prop(scene, "intg_pbgi_surfel_far_splat_t", text = "Far Splat Type")
            col.prop(scene, "intg_pbgi_surfel_near_splat_t", text = "Near Splat Type")
            col.prop(scene, "intg_pbgi_surfel_near_threshold", text = "Near Surfel Threshold")
            col.separator()
            col.prop(scene, "intg_pbgi_indirect", text= "Indirect Only")
